/*
 * TN5250j Logging & Diagnostics Pairwise Test Suite
 *
...
/**
 * Pairwise JUnit 4 test suite for TN5250j logging and diagnostics.
...
public class LoggingDiagnosticsPairwiseTest {
    ...
    enum LogLevel {
        DEBUG(TN5250jLogger.DEBUG, "DEBUG"),
        INFO(TN5250jLogger.INFO, "INFO"),
        WARN(TN5250jLogger.WARN, "WARN"),
        ERROR(TN5250jLogger.ERROR, "ERROR"),
        FATAL(TN5250jLogger.FATAL, "FATAL");
    ...
    public void setUp() throws Exception {
        ...
        testClassName = "org.tn5250j.logging.LoggingDiagnosticsPairwiseTest-" +
                       logLevel.name + "-" + logTarget.target + "-" +
                       contentType.type + "-" + format.format + "-" + filterType.type;
        logger = TN5250jLogFactory.getLogger(testClassName);
    ...
    public void testLogMessageRoutingByTargetAndLevel() {
        ...
        if (logLevel.level > TN5250jLogger.DEBUG) {
    ...
    public void testCategoryFiltering() {
        ...
        memoryCapture.logWithCategory("org.tn5250j.protocol", protocolContent);
        memoryCapture.logWithCategory("org.tn5250j.screen", screenContent);

        List<String> protocolLogs = memoryCapture.filterByCategory("org.tn5250j.protocol");
    ...
    public void testLogInjectionPrevention() {
        if (logLevel.level > TN5250jLogger.INFO) {
    ...
    public void testThrowableLogging() {
        if (logLevel.level > TN5250jLogger.ERROR) {
