...
public class Stream5250Test {
    ...
    @BeforeEach
    public void setUp() {
        buffer = createMinimalBuffer();
        stream = new Stream5250(buffer);
    }
    ...
    public void testGetNextByte_SucceedsWhenPosIsBeforeBufferLength() {
        ...
        stream.pos = buffer.length - 1;
        byte expectedValue = (byte) 0x99;
        buffer[buffer.length - 1] = expectedValue;

        // Act: Read the last valid byte
        byte result = stream.getNextByte();

        // Assert: Should succeed and return the byte
        assertEquals(expectedValue, result,"Should read the last valid byte in buffer");
        assertEquals(buffer.length, stream.pos,"Position should be incremented after read");
    }
    ...
    public void testGetByteOffset_ThrowsExceptionOnNegativeResultingIndex() throws Exception {
        ...
        assertThrows(Exception.class, () -> stream.getByteOffset(-1),
                "Expected Exception when resulting index is negative");
    }
    ...
    public void testGetByteOffset_ThrowsOnLargeNegativeOffset() throws Exception {
        ...
        assertThrows(Exception.class, () -> stream.getByteOffset(-6),
                "Expected Exception on large negative offset");
    }
    ...
    public void testGetByteOffset_ThrowsWithZeroPositionAndNegativeOffset() throws Exception {
        ...
        assertThrows(Exception.class, () -> stream.getByteOffset(-1),
                "Cannot use negative offset from position 0");
    }
    ...
    public void testGetByteOffset_SucceedsWithValidPositiveOffset() throws Exception {
        ...
        buffer[8] = expectedValue;  // pos(5) + off(3) = 8

        // Act: Read with valid positive offset
        byte result = stream.getByteOffset(3);

        // Assert: Should succeed
        assertEquals(expectedValue, result,"Should read byte at valid positive offset");
    }
    ...
    public void testGetByteOffset_SucceedsAtLastByteWithPositiveOffset() throws Exception {
        ...
        // Assert: Should succeed
        assertEquals(expectedValue, result,"Should read at exact position with zero offset");
    }
    ...
    public void testGetByteOffset_ThrowsWhenBufferIsNull() throws Exception {
        ...
        assertThrows(Exception.class, () -> stream.getByteOffset(0),
                "Expected Exception for null buffer");
    }
    ...
    public void testGetNextByte_SequentialReadsNearBoundary() {
        ...
        stream.pos = buffer.length - 2;
        byte byte1 = (byte) 0x11;
        byte byte2 = (byte) 0x22;
        buffer[buffer.length - 2] = byte1;
        buffer[buffer.length - 1] = byte2;

        // Act & Assert: Read the last two bytes successfully
        assertEquals(byte1, stream.getNextByte(),"Read second-to-last byte");
        assertEquals(buffer.length - 1, stream.pos,"Position should be MINIMAL_BUFFER.length - 1");

        assertEquals(byte2, stream.getNextByte(),"Read last byte");
        assertEquals(buffer.length, stream.pos,"Position should be MINIMAL_BUFFER.length");
        ...
        try {
            ...
            // Expected
            assertTrue(e.getMessage().contains("Buffer length exceeded"),"Exception message should mention buffer exceeded");
        }
    }
